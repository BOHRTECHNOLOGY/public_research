...
def run_single_test(learner_params, training_params, matrices, gates_structure, use_embedding):
    log = {'Trace': 'trace'}
    max_cut_solver = MaxCutSolver(learner_params, training_params, matrices, gates_structure, use_embedding, log=log)

    final_cost, all_probs = max_cut_solver.train_and_evaluate_circuit(verbose=False)
    return final_cost, all_probs


def main(matrix, run_id=0, use_ng=1, use_embedding=0):
    use_s = 1
    use_d = 1
    ...
    print("Starting", use_s, use_d, use_ng, use_embedding, run_id)
    model_dir = "logsAuto2_s_{0}_d_{1}_ng_{2}_em_{3}_run_{4}".format(use_s, use_d, use_ng, use_embedding, run_id)
    training_params['model_dir'] = model_dir
    cost, all_probs = run_single_test(learner_params, training_params, matrices, gates_structure, use_embedding)
