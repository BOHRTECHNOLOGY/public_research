...
def _qcv_model_fn(features, labels, hyperparams, mode, adj_matrices):
    """
    ...
    if task == 'optimization':
        ...
        for adj_matrix in adj_matrices:
            outps.append(circuit(adj_matrix))
        ...
        outps = circuit(adj_matrices)

    #TODO: it's a hack, not sure if it doesn't break something
    circuit_outputs = {"outputs": outps[0]}
    ...
    if mode in [tf.estimator.ModeKeys.EVAL, tf.estimator.ModeKeys.TRAIN]:
        ...
        if task == 'optimization':
            loss = myloss(circuit_outputs=outps, adj_matrices=adj_matrices)
...
class CircuitLearner(Estimator):
    ...
    def __init__(self, adj_matrices, hyperparams, model_dir=None, config=None):
        _check(hyperparams)
