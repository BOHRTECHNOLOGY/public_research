...
class ResultSet(object):
    ...
    def decode_solution_from_sample(self, sample):
        ...
        solution = Solution()
        starting_qubit = 0
        number_of_nodes = len(self.problem.outposts)
        visited_nodes = set()
        ...
        # finally we validate completeness of solution. Every route so far is valid
        # but do they add up so we visit every node?
        if len(visited_nodes) != len(self.problem.outposts):
            return None
