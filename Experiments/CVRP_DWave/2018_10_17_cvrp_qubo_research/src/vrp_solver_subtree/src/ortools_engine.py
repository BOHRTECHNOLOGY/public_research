...
def add_capacity_constraints(routing, vehicles, demand_evaluator):
    ...
    routing.AddDimensionWithVehicleCapacity(
        ...
        vehicles['capacity'].values,
        True, # start cumul to zero
        "capacity")


def add_distance_constraints(routing, distance_evaluator, max_distance):
    """Adds max total distance constraint"""
    distance = 'distance'
...
def calculate_routes(outposts, vehicles, graph, max_route_distance=200000, starting_point=0, calculation_time=5):
    ...
    add_distance_constraints(routing, edge_evaluator, max_route_distance)
    # Solve the problem.
    ...
    for idx, vehicle in vehicles.iterrows():
        # Iterate over vehicles but we can't assume anything about
        # vehicle id which can be arbitrary.
        vehicle_id = int(vehicle.vehicle_id)
        route = build_vehicle_route(routing, assignment, outposts, idx)
        if route:
