...
class DWaveEngine(object):
    ...
    def solve(self, problem, **params):
        ...
        best_cost = None
        best_result = None
        best_raw_solutions = None
        vehicle_partitions = generate_partitions(problem.outposts, problem.vehicles)
        total_qpu_time = 0
        for partition in vehicle_partitions:
            problem = problem._replace(vehicles_partition=partition)
            raw_solutions = self._get_raw_solutions(problem, **params)
            result = ResultSet(raw_solutions.record, problem)
            total_qpu_time += raw_solutions.info['timing']['total_real_time']/10e6
            if best_cost is None or result.best_solution.total_cost() < best_cost:
                best_result = result
                best_raw_solutions = raw_solutions
        best_raw_solutions.info['timing']['total_qpu_time'] = total_qpu_time
        return best_result.best_solution, best_result.number_of_best_samples, best_raw_solutions.info
...
def generate_partitions(outposts, vehicles):
