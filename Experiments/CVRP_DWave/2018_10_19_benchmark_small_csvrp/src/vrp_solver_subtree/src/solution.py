...
class ResultSet(object):
    ...
    def decode_solution_from_sample(self, sample):
        ...
        # Check if we visit a city more than once 
        all_nodes_visited = []
        for partial_solution in solution:
            all_nodes_visited += partial_solution.route

        reduced_nodes_visited = list(filter(lambda a: a != 0, all_nodes_visited))
        for node in range(1, number_of_nodes):
            if reduced_nodes_visited.count(node) != 1:
                return None
